...
contract BavaCompoundVault_VariableUpgradeable is
    Initializable,
    UUPSUpgradeable,
    BRTERC20Upgradeable
{
    ...
    function initVault(
        ...
        vaultInfo.stakingContract = _stakingContract;
    ...
    function setAllowances(uint256 _amount) external onlyOwner {
        ...
        if (address(router) != address(0)) {
            ...
            IERC20Upgradeable(IPair(address(vaultInfo.lpToken)).token0())
                .approve(address(router), _amount);
            IERC20Upgradeable(IPair(address(vaultInfo.lpToken)).token1())
                .approve(address(router), _amount);
